  ...
  async receberPedidos(req, res) {
    ...
    try {
      if (data === null) {
        return res
          .status(400)
          .json({ message: `Não foi possivel obter dados` });
      }
      ...
      if (!novoPedido.id) {
        throw new Error(
          "Erro ao criar perguntas pois nao foi possivel obter a ID"
        );
      }
      ...
      const novoProdutoPedido =
        await this.produtoPedidosController.CriarProdutosPedidos(
          produtosComVinculo,
